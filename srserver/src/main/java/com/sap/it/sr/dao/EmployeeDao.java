...
public class EmployeeDao extends BaseDao<Employee> {
	...
	public EmpInfo getEmpInfo(String id) {
		...
		User admin = getAdminInfo();
		if (admin.getPassword() != null && "".equals(admin.getPassword())) {
			String pwd = EncryptHelper.decrypt(admin.getPassword());
			rlt = LdapHelper.getEmployee(id, admin.getUserName(), pwd);
		}
		return rlt;
	}
	...
	public User getAdminInfo() {
		...
		List<User> userList = em
				.createQuery("select t from User t where length(t.password)>0 and t.role=?1", User.class)
